...
/obj/structure/roguemachine/titan/proc/give_tax_popup(mob/living/carbon/human/user)
	...
	if(newtax)
		...
		priority_announce("The new tax in Eldermoor shall be [newtax] percent.", "The Generous [user.get_role_title()] Decrees", 'sound/misc/alert.ogg', "Captain")
...
/obj/structure/roguemachine/titan/proc/make_outlaw(mob/living/carbon/human/user, raw_message)
	...
	if(raw_message in GLOB.outlawed_players)
		...
		priority_announce("[raw_message] is no longer an outlaw in Eldermoor lands.", "The [user.get_role_title()] Decrees", 'sound/misc/alert.ogg', "Captain")
		return FALSE
	var/found = FALSE
