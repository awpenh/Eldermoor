...
/obj/item/paper/scroll/cargo/proc/rebuild_info()
	...
	if(signedname)
		info += "<font size=\"2\" face=\"[FOUNTAIN_PEN_FONT]\" color=#27293f>[signedname] the [signedjob] of Eldermoor</font>"
	...
	if(istype(P, /obj/item/natural/thorn) || istype(P, /obj/item/natural/feather))
		...
		if(user.can_read(src))
			...
			if(user.mind.assigned_role == "Merchant")
				...
				playsound(src, 'sound/items/write.ogg', 100, FALSE)
				desc = "A small, palm-fitting bound scroll that can be sent by mail to the Guild. Most of the fine print is unintelligible, save for one bold SIGNEE: [signee], [signeejob] of Albia."
				signed = 1
				return
		else
			return
