...
/datum/preferences
	...
	var/static/datum/patron/default_patron = /datum/patron/veneration/lythios
...
//						detail_color = new_underwear_color
				...
				if("species")
					...
					var/result = input(user, "Select a species", "Eldermoor") as null|anything in crap
				...
				if("charflaw")
					...
					var/result = input(user, "Select a flaw", "Eldermoor") as null|anything in flawslist
					if(result)
						...
						if(S)
							for(var/i=1, i<=max_save_slots, i++)
								...
								if(!name)
									name = "Slot[i]"
								choices[name] = i
					var/choice = input(user, "CHOOSE A HERO","ELDERMOOR") as null|anything in choices
					if(choice)
